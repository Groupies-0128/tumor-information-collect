        ...
        for i in range(total_tumor_num + 1):
            ...
            for j in range(start_slice_list[i], end_slice_list[i] + 1):
                ...
                for k in range(len(slice_information[j])):
                    if slice_information[j][k][2] == i:
                        location.append(j)
                        location.append([slice_information[j][k][3] + slice_information[j][k][5] / 2, slice_information[j][k][4] + slice_information[j][k][6] / 2])
                        # [(slice_information[j][k][3], slice_information[j][k][4]), (slice_information[j][k][3] + slice_information[j][k][5], slice_information[j][k][4] + slice_information[j][k][6])]
...
def show_in_table():
    tb = pt.PrettyTable()
    tb.field_names = ["tumor NO.", "tumor size", "slices with tumor", "tumor size per slice", "tumor location"]
    for i in range(total_tumor_num + 1):
        # print(i)
        ...
        tb.add_row([i, total_tumor_size_list[i], end_slice_list[i] + 1 - start_slice_list[i], tumor_no_size_per_slice_list[i], tumor_no_location_list[i]])
    print(tb)
